...
import VoltSearchResultsMap from "./results/search-results-map";
import VoltSearchDetail from "./search-details";
...
const VoltDesktopSearchWrapper: FC<IVoltDesktopSearchWrapper> = ({ children, formState }) => {
  ...
  return (
    <div className="grid grid-cols-[min(39vw,_492px)_1fr] h-screen">
      <div className="w-full h-full grid grid-rows-[minmax(0,_max-content)_1fr_minmax(0,_max-content)] overflow-hidden">
          ...
          <ScrollArea className="h-full [&>div>div:first-child]:h-full">
            <div className="px-7 xl:px-9 2xl:px-11 space-y-8 flex flex-col justify-between h-full">
              <div className="h-full space-y-8">
                <VoltSearchDetail formState={formState} />
                {children}
              </div>
              <div className={cn("mt-auto", !voltAtomValue.isSearchResultsVisible ? "pb-6 2xl:pb-8" : "pb-4 2xl:pb-6 pt-6")}>
        ...
        <div>
          {voltAtomValue.isSearchResultsVisible && (
      ...
      <div className="w-full h-full">
        {voltAtomValue.isSearchResultsVisible ? (
          <VoltSearchResultsMap key={searchParams.toString()} />
        ) : (
...
export default VoltDesktopSearchWrapper;
