...
].map((el) => ({ ...el, pricePerAcre: Number((el.price / el.acreage).toFixed(2)) }));
...
const CalculatePrice = ({ price }: { price: number }) => {
  ...
  return (
    <div>
      ...
      <h2 className="text-2xl xl:text-4xl font-bold">{numFormatter.format(price)}</h2>
    </div>
  );
};
      ...
      >
        ...
        {data.map((property) => (
          <FaLocationDot
            ...
            className="cursor-pointer absolute top-0 -translate-y-full text-[#F78290] size-5 xl:size-6 -translate-x-1/2 hover:scale-150 hover:text-[#FF2F48] transition-all duration-100"
          />
        ))}
      </div>
    ...
    <div className="py-3 xl:py-0 border-t border-t-gray-200 xl:border-y-0 flex gap-4">
      <div className="space-y-1">
        <p className="xl:hidden text-xs font-medium text-gray-600">
          Min Price Per Acre: <span className="text-black font-semibold">{numFormatter.format(20000)}</span>
        </p>
        <p className="xl:hidden text-xs font-medium text-gray-600">
          Average Price Per Acre: <span className="text-black font-semibold">{numFormatter.format(20000)}</span>
        </p>
        <p className="xl:hidden text-xs font-medium text-gray-600">
          Max Price Per Acre: <span className="text-black font-semibold">{numFormatter.format(20000)}</span>
        </p>
      </div>
