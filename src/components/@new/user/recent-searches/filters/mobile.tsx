...
import { parseSearchParams, updateSearchParamsWithFilters } from "@/lib/utils";
  ...
  return (
    <>
        ...
        <div className="bg-white md:shadow-4 h-[80vh] w-full overflow-y-auto overflow-x-hidden flex rounded-lg flex-col ">
          ...
          <Accordion type="single" collapsible className="w-full px-5 md:px-8">
            <AccordionItem value="states">
              ...
              <AccordionContent>
                <RadioGroup
                  onValueChange={(value) => changeFilter([{ key: "state", value, resetKey: "county" }])}
                  value={values?.state || ""}
                  className=""
                >
                  {states.map((state) => (
            ...
            >
              ...
              <AccordionContent>
                <RadioGroup onValueChange={(value) => changeFilter([{ key: "county", value }])} value={values?.county || ""} className="">
                  {counties.map((state) => (
                    <RadioGroupItem key={state.value} value={state.value} label={state.label} />
                  ))}
                </RadioGroup>
            ...
            <AccordionItem value="acreage">
              ...
              <AccordionContent>
                <MinmaxDropdownContent
                  ...
                  disablePadding
                  value={{
                    min: typeof values?.acreageMin === "number" || values?.acreageMin === 0 ? values.acreageMin : null,
                    max: typeof values?.acreageMax === "number" || values?.acreageMax === 0 ? values.acreageMax : null,
                  }}
                  renderOption={(value) => {
            ...
            <AccordionItem value="voltPrice">
              ...
              <AccordionContent>
                <MinmaxDropdownContent
                  ...
                  inputPrefix="$"
                  value={{
                    min: typeof values?.voltPriceMin === "number" || values?.voltPriceMin === 0 ? values.voltPriceMin : null,
                    max: typeof values?.voltPriceMax === "number" || values?.voltPriceMax === 0 ? values.voltPriceMax : null,
                  }}
                  renderOption={(val) => {
                    if (val?.min && val?.max) {
          ...
          <div className="flex flex-col-reverse md:justify-end md:flex-row gap-3 w-full sticky bottom-0 bg-white mt-auto py-4 md:py-6 px-5 md:px-8">
            ...
            <Button
              ...
              onClick={() => {
                setOpen(true);
                const res = userRecentSearchesValidations.safeParse(values);
                if (res.data) {
                  const params = new URLSearchParams();
                  Object.keys(res.data).forEach((key) => {
                    if (key) {
                      params.set(key, res.data[key as keyof typeof res.data]!.toString());
                    }
                  });
      ...
      <div className="2xl:hidden ml-auto">
        <Button className="p-2.5 h-fit !bg-transparent text-grey-800 border border-grey-100 !rounded-xl" onClick={() => setOpen(true)}>
