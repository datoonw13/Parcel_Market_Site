...
const LandsDesktopFilters: FC<LandsDesktopFiltersProps> = ({ filters, setFilters, select, totalCount }) => {
  const { counties, getCountiesByState, getCounty, getState, states } = useStates({ hideBlackListedStated: false });
  return (
    <div className="flex flex-col gap-7">
      <div className="grid gap-3 grid-cols-4">
        ...
        <AutoComplete
          ...
          options={filters.state ? getCountiesByState(filters.state) || [] : []}
          getOptionLabel={(item) => item.label}
          getOptionKey={(item) => item.value}
          onChange={(item) => setFilters({ ...filters, county: item?.value || null })}
          placeholder="County"
          value={filters.state && filters.county && getCounty(filters.state, filters.county)?.short}
          ...
          <div className="flex items-center gap-3">
            ...
            {select.totalSelected > 0 && (
              <Button
                ...
                variant="secondary"
                // variant="text"
                className="!bg-white w-8 h-8 rounded-full hover:bg-error-100 flex items-center justify-center group focus:bg-error"
              >
