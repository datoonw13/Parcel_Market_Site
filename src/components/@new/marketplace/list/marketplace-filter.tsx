  ...
  return (
    <div className="mb-6 md:mb-8 lg:mb-10">
      <div className="flex justify-between items-center gap-2">
        <MarketPlaceDesktopFilters disabled={!user?.planSelected} />
        <MarketPlaceMobileFilters disabled={!user?.planSelected} />
        <div className="flex items-center gap-3 justify-end">
          <p className="hidden sm:block text-grey-600 text-xs">{data?.pagination.totalCount} Lands</p>
          <Sort disabled={!user?.planSelected} options={SortEnum} />
        </div>
      </div>
    </div>
  );
