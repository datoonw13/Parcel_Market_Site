...
const AccountActivationPage = async ({
  searchParams,
}: {
  searchParams: { token?: string; email?: string; redirectUrl?: string; landOwner?: string };
}) => {
  ...
  if (data) {
    ...
    redirect(
      `${searchParams.landOwner ? routes.auth.signUp.successLandOwner.fullUrl : routes.auth.signUp.success.fullUrl}?${params.toString()}`
    );
  }
